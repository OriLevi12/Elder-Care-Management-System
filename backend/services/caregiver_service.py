...
def add_caregiver_service(caregiver: CaregiverCreate, db: Session) -> CaregiverResponse:
    """
    ...
    """
    # Check if caregiver with same ID already exists
    ...
    delete_from_cache("caregiver_list")  # Clear the list cache
    delete_from_cache(f"caregiver_{new_caregiver.id}")  # Clear individual cache

    return new_caregiver

def get_all_caregivers_service(db: Session) -> list[CaregiverResponse]:
    """
    Retrieve all caregivers with Redis caching.
    
    Cache Strategy:
    - Cache key: "caregiver_list"
    ...
    """
    cache_key = "caregiver_list"

    # Try to get data from Redis cache first
    cached_data = get_from_cache(cache_key)
    if cached_data:
        print("✅ Retrieved caregiver list from Redis cache")
    ...
    print("📦 Retrieved caregiver list from DB and storing in Redis")
    set_in_cache(cache_key, [c.dict() for c in result], ttl=300)

    return result

def get_caregiver_by_id_service(caregiver_id: int, db: Session) -> CaregiverResponse:
    """
    Retrieve a specific caregiver by ID with Redis caching.
    
    Cache Strategy:
    - Cache key: "caregiver_{caregiver_id}"
    - TTL: 300 seconds (5 minutes)
    - On cache hit: Return cached data
    - On cache miss: Query DB, cache result, return data
    ...
    """
    cache_key = f"caregiver_{caregiver_id}"
    
    # Try to get data from Redis cache first
    cached_data = get_from_cache(cache_key)
    if cached_data:
        print(f"✅ Retrieved caregiver {caregiver_id} from Redis cache")
    ...
    print(f"📦 Retrieved caregiver {caregiver_id} from DB and storing in Redis")
    set_in_cache(cache_key, result.dict(), ttl=300)
    
    return result

def update_caregiver_salary_service(caregiver_id: int, salary_update: CaregiverUpdateSalary, db: Session) -> CaregiverResponse:
    """
    ...
    Raises:
        HTTPException: If caregiver not found
    """
    # Find and verify caregiver exists
    caregiver = db.query(Caregiver).filter(Caregiver.id == caregiver_id).first()
    if not caregiver:
        raise HTTPException(status_code=404, detail="Caregiver not found")
    ...
    # Invalidate related caches to ensure data consistency
    delete_from_cache("caregiver_list")  # Clear the list cache
    delete_from_cache(f"caregiver_{caregiver_id}")  # Clear individual cache
    
    return caregiver

def generate_caregiver_pdf_service(caregiver_id: int, db: Session) -> str:
    """
    ...
    Raises:
        HTTPException: If caregiver not found
    """
    # Get caregiver data (this will use cache if available)
    caregiver = get_caregiver_by_id_service(caregiver_id, db)
    
    # Generate PDF using the utility function
    filename = generate_caregiver_pdf(caregiver)
    return filename

def delete_caregiver_service(caregiver_id: int, db: Session) -> dict:
    """
    Delete a caregiver by ID with Redis cache invalidation.
    ...
    Raises:
        HTTPException: If caregiver not found
    """
    # Check if caregiver exists before deletion
    caregiver = db.query(Caregiver).filter(Caregiver.id == caregiver_id).first()
    if not caregiver:
        raise HTTPException(status_code=404, detail="Caregiver not found")
